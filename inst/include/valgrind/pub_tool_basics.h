...
//
// VG_ is for symbols exported from modules.  ML_ (module-local) is——— VG_开头的符号是用于从模块中导出的，ML_开头的符号为对外部模块不可见的，
// for symbols which are not intended to be visible outside modules,-----但是因为需要对C文件给定模块之间可见，所以不能定义成静态变量
