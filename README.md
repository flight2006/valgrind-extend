...
2.如果你已经安装官方valgarind，只需下载对应插件文件夹，然后按照官方指南修改项目主目录makefile和config文件，以及插件目录下makefile文件，然后编译安装。
##使用：
像Valgrind其他功能插件一样，使用命令行valgrind --tool='toolname' ./'binary'
其中'toolname'为插件名，对应为integer和sconvcheck，'binary'为待测程序编译后的二进制文件
------------------------------------------------------------------------------
